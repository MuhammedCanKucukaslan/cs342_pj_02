...
pthread_cond_t sim_start_cond; // the scheduler thread will wait for this condition variable to initiate scheduling
pthread_cond_t initial_threads_created_cond;// bound to the queue mutex, may not be needed TODO
...
enum BOOL chk_initial_threads_in_rdy_q();
...
int main(int argc, char **argv)
{
    ...
    // the program will be initiated successfully regardless of whether the following code
    // executed before the sched_thread or after!
    // usleep(100*1000);
    /*
    pthread_mutex_lock(&sim_start_mutex);
    while (scheduler_started < 0) {
        pthread_cond_wait(&initial_threads_created_cond, &sim_start_mutex);
    }
    */
    ...
    pthread_cond_destroy(&sim_start_cond);
...
void *p_sched(void *arg)
{
    ...
    pthread_cond_signal(&initial_threads_created_cond);
    ...
    while (running_process_count < 1 || allp_count < 1) {
        pthread_cond_signal(&initial_threads_created_cond);
        pthread_cond_wait(&sim_start_cond, &running_process_count_mutex);
    }
    pthread_mutex_unlock(&running_process_count_mutex);
...
void *process(void *arg)
{
    ...
    do {
        ...
        elapsed_time = (current_time.tv_sec - sim_start_date.tv_sec) * 1000 +
                       (current_time.tv_usec - sim_start_date.tv_usec) / 1000;
        if (outmode == 3)
            printf("%ld Process %ld added to ready queue.\n", elapsed_time, pid);
        ...
        if (outmode == 2) {
            ...
            printf("%ld %ld %s, burstlength = %d, sleep_time: %d\n", elapsed_time, my_pcb.pid,
        ...
        elapsed_time = (current_time.tv_sec - sim_start_date.tv_sec) * 1000 +
                       (current_time.tv_usec - sim_start_date.tv_usec) / 1000;
        if (outmode == 3)
            printf("%ld Process %ld finished its burst.\n", elapsed_time, pid);
        pthread_mutex_unlock(&cpu_mutex);
        ...
        if (my_pcb.remaining_burst_length == 0) {
            ...
            if (rn < p0) {
                ...
                if (outmode == 3)
                    printf("%ld Process %ld shall be terminated.\n", elapsed_time, pid);
            } else if (rn < p0 + p1) {
                my_pcb.state = WAITING;
                if (outmode == 3)
                    printf("%ld Process %ld shall be put to waiting queue of DEVICE1, wait: %d.\n", elapsed_time, pid,
                           io1_wait_count);
                ...
                if (outmode == 2) {
                    ...
                    printf("%ld %ld USING DEVICE1\n", elapsed_time, my_pcb.pid);
                } else if (outmode == 3) {
                    printf("%ld %ld USING DEVICE1, T1 = %d\n", elapsed_time, my_pcb.pid, T1);
                }
                ...
                if (outmode == 3)
                    printf("%ld Process %ld shall be put to waiting queue of DEVICE2, wait: %d.\n", elapsed_time, pid,
                           io2_wait_count);
                my_pcb.state = WAITING;
                ...
                if (outmode == 2) {
                    //  print out the current time (ms), its pid, and the device that it is using.
                    printf("%ld %ld USING DEVICE2\n", elapsed_time, my_pcb.pid);
                } else if (outmode == 3) {
                    printf("%ld %ld USING DEVICE2, T2 = %d\n", elapsed_time, my_pcb.pid, T2);
                }
                // simulate the IO run
            ...
            if (my_pcb.state != TERMINATED) {
                ...
                elapsed_time = (current_time.tv_sec - sim_start_date.tv_sec) * 1000 +
                               (current_time.tv_usec - sim_start_date.tv_usec) / 1000;
                if (outmode == 3)
                    printf("%ld Process %ld is not terminated, burslength %d ms\n", elapsed_time, my_pcb.pid,
...
void mutex_queue_add(struct PCB pcb)
{
    ...
    // signal sched for initial threads, important only at startup
    pthread_cond_signal(&initial_threads_created_cond);
    // unlock mutex
    pthread_mutex_unlock(&pcb_queue_mutex);
}
...
enum BOOL chk_initial_threads_in_rdy_q()
{
    enum BOOL initial_threads_in_rdy_q = TRUE;
    pthread_mutex_lock(&pcb_queue_mutex);
    if (ready_queue.count < 10 && ready_queue.count < MAXP) {
        initial_threads_in_rdy_q = FALSE;
    }
    pthread_mutex_unlock(&pcb_queue_mutex);
    return initial_threads_in_rdy_q;
}
